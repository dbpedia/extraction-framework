...
class DynamicExtractionManager(
  ...
  def mappingExtractor(language: Language) = synchronized {
    _mappingTestExtractors(language)
  }

  def customExtractor(language: Language) = synchronized {
    _customTestExtractors(language)
  }

  def ontology() = synchronized {
    _ontology
  }
  ...
  def ontologyPages() = synchronized {
    _ontologyPages
  }
  ...
  def mappingPageSource(language: Language) = synchronized {
    _mappingPages(language).values
  }

  def mappings(language: Language): Mappings = synchronized {
    _mappings(language)
  }

  def rmlStatistics: RMLStatisticsHolder = synchronized {
    _rmlStatistics
  }
