...
class Mappings(@PathParam("lang") langCode: String) {
  ...
  def get: Elem = {
    <html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en">
      {ServerHeader.getHeader("Mappings")}<body>
      <div class="row">
  ...
  def getPages: Elem = {
    <html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en">
      {ServerHeader.getHeader("Mapping pages")}<body>
      <div class="row">
        <div class="col-md-3 col-md-offset-5">
          <h2>Mapping pages</h2>{Server.instance.extractor.mappingPageSource(language).map(page => PageUtils.relativeLink(parser(page).getOrElse(throw new Exception("Cannot get page: " + page.title.decoded + ". Parsing failed"))) ++ <br/>)}
        </div>
      </div>
    </body>
    </html>
    ...
    <html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en">
      ...
      <div class="row">
        <div class="col-md-3 col-md-offset-5">
          ...
          <br/>
          <br/>{pages.map(page => PageUtils.relativeLink(parser(page).getOrElse(throw new Exception("Cannot get page: " + page.title.decoded + ". Parsing failed"))) ++ <br/>)}
        </div>
      </div>
    </body>
    </html>
      ...
      override def run(): Unit = {
        ...
        try {
          ...
          for (title <- titles) {
            ...
            val mappingAsString = try {
              getRdfMapping(title.encodedWithNamespace.replace(":", "%3A").replace("/", "%2F"))
            }
            catch {
              case x: Throwable => ""
            }
  ...
  def validate: Elem = {
    <html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en">
      {ServerHeader.getHeader("Validate Mappings")}<body>
      <div class="row">
        <div class="col-md-3 col-md-offset-5">
          <h2>Validate Mappings</h2>
          <p>
            <a href="*">Validate all mappings</a>
          </p>{Server.instance.extractor.mappingPageSource(language).map(page => PageUtils.relativeLink(parser(page).getOrElse(throw new Exception("Cannot validate mapping: " + page.title.decoded + ". Parsing failed"))) ++ <br/>)}
        </div>
      </div>
    </body>
    </html>
  ...
  def extractionSamples: Elem = {
    <html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en">
      ...
      <div class="row">
        <div class="col-md-3 col-md-offset-5">
          <h2>Mapping pages</h2>{Server.instance.extractor.mappingPageSource(language).map(page => PageUtils.relativeLink(parser(page).getOrElse(throw new Exception("Cannot read page: " + page.title.decoded + ". Parsing failed"))) ++ <br/>)}
        </div>
      </div>
    </body>
    </html>
  ...
  def mappedRedirects: Elem = {
    <html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en">
      {ServerHeader.getHeader("Mapped Templates Redirects", true)}<body>
      ...
      <table class="tablesorter table myTable" align="center" style="width:500px; margin:auto">
        ...
        for ((redirect_from, redirect_to) <- reversedRedirects) yield {
          ...
          if (pages.contains(rf.decoded)) {
            <tr>
              <td>
                <a href={"http://mappings.dbpedia.org/index.php/Mapping_" + langCode + ":" + rf.decoded}>
                  {rf.decoded}
                </a>
              </td>
              <td>
                <a href={"http://mappings.dbpedia.org/index.php/Mapping_" + langCode + ":" + rt.decoded}>
                  {rt.decoded}
                </a>
              </td>
              <td>
