...
class SparkExtractionJob( extractors: Seq[Class[_ <: Extractor[_]]],
    ...
    try {
      ...
      while (!readerFinished || pageBuffer.nonEmpty) {
        if (pageBuffer.lengthCompare(chunkSize) >= 0 || readerFinished) {
          ...
          sparkContext.parallelize((0 until nextChunkSize).map(_ => pageBuffer.dequeue))
          // WikiPage => ExtractionResult
            .mapPartitions(pages => {
              //create extractor from broadcast values
              ...
              pages.map(Methods.processPage(bc_namespaces.value, localExtractor, _))
            })
          ...
          // Process Results
            .foreach(result => {
              ...
              if (times.lengthCompare(1000) >= 0) {
                ...
                times.clear()
                extractionRecorder.printLabeledLine(s"finished extraction of pages ${(chunkNumber * chunkSize) - (1000 * chunkSize) + 1} to ${chunkNumber * chunkSize} with  ~$average ms per page", RecordSeverity.Info, lang)
              }
              chunkNumber += 1
            })
        }
  ...
  def processPage(namespaces: Set[Namespace], extractor: Extractor[WikiPage], page : WikiPage): ExtractionResult = {
    try {
      // Generate Logs
      ...
      if (namespaces.exists(_.equals(page.title.namespace))) {
        ExtractionResult(Some(extractor.extract(page, page.uri)), records)
      }
      else {
  ...
  def processResult(result : ExtractionResult, destination: Destination): Unit = {
    // write
    try {
      result.quads match {
