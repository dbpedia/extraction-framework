...
object WikiInfo
{
  ...
  */
  def fromLines( lines : Iterator[String] ) : Seq[WikiInfo] =
  {
    ...
    lines.foreach(line => if (line.nonEmpty) info += fromLine(line))
  ...
  def fromLine( line : String ) : WikiInfo =
  {
      ...
      {
        case nfe : NumberFormatException => throw new Exception("expected page count in field with index [5], found line ["+line+"]")
      }
