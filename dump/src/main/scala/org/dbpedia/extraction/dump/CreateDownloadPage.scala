...
new DatasetInfo("bookmashup", "Links to RDF Bookmashup", "Links between books in DBpedia and data about them provided by the ((http://www4.wiwiss.fu-berlin.de/bizer/bookmashup/ RDF Book Mashup)). Provided by Georgi Kobilarov. Update mechanism: unclear/copy over from previous release."),
new DatasetInfo("dailymed", "Links to DailyMed", "Links between DBpedia and ((http://dailymed.nlm.nih.gov/ DailyMed)). Update mechanism: unclear/copy over from previous release."), 
new DatasetInfo("dblp", "Links to DBLP", "Links between computer scientists in DBpedia and their publications in the ((http://www.informatik.uni-trier.de/~ley/db/ DBLP)) database. Links were created manually. Update mechanism: Copy over from previous release."),
new DatasetInfo("diseasome", "Links to Diseasome", "Links between DBpedia and ((http://diseasome.eu/ Diseasome)). Update mechanism: unclear/copy over from previous release."), 
new DatasetInfo("drugbank", "Links to DrugBank", "Links between DBpedia and ((http://www.drugbank.ca/ DrugBank)). Update mechanism: unclear/copy over from previous release."), 
new DatasetInfo("eurostat", "Links to Eurostat", "Links between countries and regions in DBpedia and data about them from ((http://ec.europa.eu/eurostat Eurostat)). Links were created manually. Update mechanism: Copy over from previous release."),
new DatasetInfo("factbook", "Links to CIA Factbook", "Links between countries in DBpedia and data about them from ((https://www.cia.gov/library/publications/the-world-factbook/ CIA Factbook)). Links were created manually. Update mechanism: Copy over from previous release."),
new DatasetInfo("flickr", "Links to flickr wrappr", "Links between DBpedia concepts and photo collections depicting them generated by the ((http://www4.wiwiss.fu-berlin.de/flickrwrappr/ flikr wrappr)). Update mechanism: script in SVN."),
new DatasetInfo("freebase", "Links to Freebase", "Links between DBpedia and ((http://www.freebase.com/ Freebase)) (MIDs). Update mechanism: script in SVN."), 
new DatasetInfo("geonames", "Links to Geonames", "Links between geographic places in DBpedia and data about them in the ((http://www.geonames.org/ Geonames)) database. Provided by the Geonames people. Update mechanism: unclear/copy over from previous release."),
new DatasetInfo("gutenberg", "Links to Project Gutenberg", "Links between writers in DBpedia and data about them from ((http://www.gutenberg.org/ Project Gutenberg)). Update mechanism: script in SVN. Since this requires manual changes of files and a D2R installation, it will be copied over from the previous DBpedia version and updated between releases by the maintainers (Piet Hensel and Georgi Kobilarov)."),
new DatasetInfo("musicbrainz", "Links to MusicBrainz", "Links between artists, albums and songs in DBpedia and data about them from ((http://musicbrainz.org/ MusicBrainz)). Created manually using the result of SPARQL queries. Update mechanism: unclear/copy over from previous release."),
new DatasetInfo("nyt", "Links to New York Times", "Links between New York Times subject headings and DBpedia concepts."),
new DatasetInfo("opencyc", "Links to Cyc", "Links between DBpedia and ((http://opencyc.org/ Cyc)) concepts. ((OpenCyc Details)). Update mechanism: awk script."),
new DatasetInfo("revyu", "Links to Revyu", "Links to Reviews about things in ((http://revyu.com/ Revyu)). Created manually by Tom Heath. Update mechanism: unclear/copy over from previous release."),
new DatasetInfo("sider", "Links to SIDER", "Links between DBpedia and ((http://sideeffects.embl.de/ SIDER)). Update mechanism: unclear/copy over from previous release."),
new DatasetInfo("tcm", "Links to TCMGeneDIT", "Links between DBpedia and ((http://tcm.lifescience.ntu.edu.tw/ TCMGeneDIT)). Update mechanism: unclear/copy over from previous release."),
new DatasetInfo("uscensus", "Links to US Census", "Links between US cities and states in DBpedia and data about them from US Census. Update mechanism: unclear/copy over from previous release."),
new DatasetInfo("wikicompany", "Links to WikiCompany", "Links between companies in DBpedia and companies in ((http://wikicompany.org/ Wikicompany)). Update mechanism: script in SVN."),
...
new DatasetInfo("wordnet", "WordNet Classes", "Classification links to ((http://www.w3.org/TR/wordnet-rdf/ RDF representations)) of ((http://wordnet.princeton.edu/ WordNet)) classes. Update mechanism: unclear/copy over from previous release.")
)
...
def createWikiTable(header: String, datasets: Seq[DatasetInfo], filespecs: String) {
  ...
  for (dataset <- datasets) {
    ...
    for (language <- languages) {
      if (first) {
        print("||((#"+dataset.title.replaceAll(" ","")+' '+dataset.title+")) "+preview(version+"/"+language+"/"+dataset.dataset+"_"+language.wikiCode+".nt.bz2")+"|")
        first = false
      }
