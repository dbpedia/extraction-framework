...
import org.dbpedia.extraction.sources.{Source, Source2}
...
class Redirects2(val map : Map[String, String]) extends java.io.Serializable
{
    /**
     * Resolves a redirect.
     *
...
object Redirects2
{
     ...
     */
    def load(source : Source2, cache : File, lang : Language) : Redirects2 =
    {
     ...
     */
    private def loadFromCache(cache : File) : Redirects2 =
    {
        logger.info("Loading redirects from cache file "+cache)
        val inputStream = new ObjectInputStream(new FileInputStream(cache))
        try
        {
            val redirects = new Redirects2(inputStream.readObject().asInstanceOf[Map[String, String]])
     ...
     */
    def loadFromSource(source: Source2, lang: Language): Redirects2 = {
        ...
        val redirects = new Redirects2(source.flatMap(redirectFinder).toMap)

        logger.info("Redirects loaded from source (" + lang.wikiCode + ")")
        redirects
    }

    private class RedirectFinder(lang : Language) extends (WikiPageWithRevisions => List[(String, String)])
    {
        ...
        override def apply(page : WikiPageWithRevisions) : List[(String, String)]=
        {
