...
import org.dbpedia.extraction.mappings.{ExtractionMonitor2, Extractor}
...
/**
  ...
  * */
class Config2(val configPath: String) extends
  Properties(Config2.universalProperties) with java.io.Serializable
{
  ...
  def getDefaultExtractionRecorder[T](lang: Language, interval: Int = 100000, preamble: String = null, writer: Writer = null, datasets : ListBuffer[Dataset] =  ListBuffer[Dataset](), monitor : ExtractionMonitor2 = null): ExtractionRecorder2[T] ={
    val w = if(writer != null) writer
      else openLogFile(lang.wikiCode) match{
        case Some(s) => new OutputStreamWriter(s)
        case None => null
      }
    new ExtractionRecorder2[T](w, interval, preamble, slackCredentials.getOrElse(null), datasets, lang, monitor)
  }
