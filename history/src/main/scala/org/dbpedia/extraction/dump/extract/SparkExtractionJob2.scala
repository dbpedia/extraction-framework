...
import org.dbpedia.extraction.wikiparser.{Namespace, WikiPageWithRevisions, WikiTitle}
...
/**
  ...
  */
class SparkExtractionJob2(extractors: Seq[Class[_ <: Extractor[_]]],
                         context: SparkExtractionContext2,
                         namespaces: Set[Namespace],
                         destination: Destination,
                         lang: Language,
                         retryFailedPages: Boolean,
                         extractionRecorder: ExtractionRecorder2[WikiPageWithRevisions]) {

  def run(spark: SparkSession, config: Config2): Unit = {
    ...
    try {
      ...
      dumpSources.foreach(file => {
         ...
         */
        val wikiPageParsedRDD : RDD[WikiPageWithRevisions] = wikiPageXmlRDD.flatMap(SerializableUtils2.parseXMLToWikiPage(_, broadcastLanguage.value))
        ...
        val extractedDataRDD : RDD[Quad] = wikiPageParsedRDD.mapPartitions(pages => {
          // TODO: ImageExtractor
          def worker_context =
            new SparkExtractionContext2 {
              def ontology: Ontology = broadcastOntology.value
              def language: Language = broadcastLanguage.value
              def redirects: Redirects2 = broadcastRedirects.value
              def disambiguations: Disambiguations = broadcastDisambiguations.value
            }
          val localExtractor = CompositeParseExtractor2.load(broadcastExtractors.value, worker_context)
          pages.map(page =>
            SerializableUtils2.extractQuadsFromPage(broadcastNamespaces.value, localExtractor, page)
          )
        }).flatMap(identity)
...
//        if (extractionRecorder.monitor != null) extractionRecorder.monitor.reportCrash(extractionRecorder, ex)
  ...
  def createRecords(page: WikiPageWithRevisions): Seq[RecordEntry2[WikiPageWithRevisions]] = {
  ...
  private def getLatestDate(finder: Finder[_], config: Config2): String = {
