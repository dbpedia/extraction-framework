...
import org.dbpedia.extraction.mappings.WikiPageWithRevisionsExtractor
import org.dbpedia.extraction.sources.{Source, Source2}
import org.dbpedia.extraction.util._
import org.dbpedia.extraction.wikiparser.{Namespace, WikiPage, WikiPageWithRevisions}
...
class ExtractionJob2(
                      ...
                      val extractionRecorder: ExtractionRecorder2[WikiPageWithRevisions])
{
  ...
  private val workers = SimpleWorkers { page: WikiPageWithRevisions =>
    try {
    ...
    try {
      ...
      if(retryFailedPages){
        ...
        for(page <- fails) {
          page.toggleRetry()
          page match{
            case p: WikiPageWithRevisions => workers.process(p)
            case _ =>
          }
        }
