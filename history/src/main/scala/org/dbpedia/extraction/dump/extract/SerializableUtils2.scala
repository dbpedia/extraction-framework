...
import org.dbpedia.extraction.wikiparser.{Namespace, WikiPage, WikiPageWithRevisions, WikiTitle}

import scala.util.Try
import scala.xml.XML.loadString

object SerializableUtils2 extends Serializable {
  ...
  /**
    ...
    */
  def extractQuadsFromPage(namespaces: Set[Namespace], extractor: Extractor[WikiPageWithRevisions], page: WikiPageWithRevisions): Seq[Quad] = {
    var quads = Seq[Quad]()
    try {
  ...
  /**
    * Parses an XML string to a wikipage.
    * based on org.dbpedia.extraction.sources.XMLSource
    * @param xmlString xml wiki page
    * @return Option[WikiPage]
    */
  def parseXMLToWikiPage(xmlString: String, language: Language): Option[WikiPageWithRevisions] = {
    /*
    ...
    try {
      ...
      if (title != null) {
        ...
        Some(new WikiPageWithRevisions(
          title     = title,
          redirect  = _redirect,
          id        = (page \ "id").text,
