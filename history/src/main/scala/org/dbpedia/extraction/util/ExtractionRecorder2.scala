...
import org.dbpedia.extraction.mappings.ExtractionMonitor2
import org.dbpedia.extraction.transform.Quad
import org.dbpedia.extraction.config.Config.SlackCredentials
import org.dbpedia.extraction.wikiparser.{PageNode, WikiPageWithRevisions, WikiTitle}
...
class ExtractionRecorder2[T](
                              ...
                              val monitor: ExtractionMonitor2 = null
                            ) {

  def this(er: ExtractionRecorder2[T]) = this(er.logWriter, er.reportInterval, er.preamble, er.slackCredantials)
  ...
  /**
   * prints a message of a RecordEntry2 if available and
   * assesses a RecordEntry2 for the existence of a Throwable and forwards
   * the record to the suitable method for a failed or successful extraction
   *
   * @param records - the RecordEntries for a WikiPage
   */
  def record(records: RecordEntry2[T]*): Unit = {
    for(record <- records) {
      //val count = increaseAndGetSuccessfulPages(record.language)
      record.page match{
        case page: WikiPageWithRevisions =>
...
class WikiPageWithRevisionsEntry(page : WikiPageWithRevisions) extends RecordEntry2[WikiPageWithRevisions](
  page = page,
  identifier = page.uri,
