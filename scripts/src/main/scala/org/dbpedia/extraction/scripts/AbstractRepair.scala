...
object AbstractRepair {
  ...
  val workers = SimpleWorkers(1.0, 1.0) { language: Language =>
    ...
    if(oldFile.exists()) {
      ...
      QuadMapper.mapQuads(language, new RichFile(faultyFile), destination, required = true, closeWriter = false) { quad =>
      ...
      QuadMapper.mapQuads(language, new RichFile(oldFile), destination, required = true) { quad =>
