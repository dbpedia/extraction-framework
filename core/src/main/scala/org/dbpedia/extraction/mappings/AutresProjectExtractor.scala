  ...
  extends PageNodeExtractor
{
  ...
  private val autresProjetsRegexList = List("autres projet.*".r)

//  "autres projet" = "sister project links"
  ...
  override def extract(node : PageNode, subjectUri : String) : Seq[Quad] =
  {
    ...
    for { template <- InfoboxExtractor.collectTemplates(node)
          ...
          if autresProjetsRegexList.exists(regex => regex.findFirstMatchIn(resolvedTitle).isDefined)
          }
    {
          ...
          for(splitNode <- splitPropertyNodes; pr <- extractValue(splitNode))
          {
            ...
            try
            {
              ...
              if (mapProjects.contains(property.key)) {
                ...
                quads += new Quad(language, DBpediaDatasets.InfoboxProperties, subjectUri, sameAsProperty, value, splitNode.sourceIri, null)
              }
            }
            catch
            {
