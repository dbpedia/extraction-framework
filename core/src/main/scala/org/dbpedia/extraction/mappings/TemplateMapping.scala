                           ...
                           def language : Language }  ) extends ClassMapping[Node]
{
    ...
    override def extract(node : Node, subjectUri : String, pageContext : PageContext) : Seq[Quad] =
    {
        ...
        // do these only once
        // TODO: what's the point of this special treatment of ConstantMappings? 
        // Performance improvement? Probably not. What else? Run them only once for each page?
        // Doesn't make sense - the triple generated by a ConstantMapping should use the subject URI
        // of the template from which its value was extracted, not the subject URI of the page.
        graph ++ constantMappings.flatMap(_.extract(node.asInstanceOf[TemplateNode], subjectUri, null))
    }

    def extractTemplate(node : TemplateNode, subjectUri : String, pageContext : PageContext) : Seq[Quad] =
    {
        ...
        pageNode.annotation(TemplateMapping.CLASS_ANNOTATION) match
        {
            case None => //So far, no template has been mapped on this page
            {
                ...
                //Extract properties
                graph ++ propertyMappings.flatMap(_.extract(node, subjectUri, pageContext))
            }
            case Some(pageClasses) => //This page already has a root template.
            {
                ...
                //Extract properties
                graph ++= propertyMappings.flatMap(_.extract(node, instanceUri, pageContext))
            }
        }
