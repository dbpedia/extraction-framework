...
object MappingsLoader {
  ...
  def load(context: {
    ...
    val logger = ExtractionLogger.getLogger(getClass, context.language)
    ...
    for (page <- context.mappingPageSource.flatMap(parser.apply(_, context.redirects));
      ...
      try {
        tnode.title.decoded match {
          case "TemplateMapping" => {
            if (!classMappings.contains(name)) {
              ...
              logger.error("Duplicate template mapping for '" + name + "' on page " + page.title.decodedWithNamespace + ".")
            }
          }
          case "TableMapping" => {
        ...
        case ex: Throwable => logger.log(Level.WARN, "Couldn't load " + tnode.title.decoded + " on page " + page.title.decodedWithNamespace + ". Details: " + ex.getMessage, ex)
      }
    }
  ...
  private def loadPropertyMappings(node: TemplateNode, propertyName: String, topLevelTemplate: String, context: {
    ...
    for (mappingsNode <- node.property(propertyName);
      ...
      try {
        ...
        case ex: Exception => logger.log(Level.WARN, "Couldn't load property mapping on page " + node.root.title.decodedWithNamespace + ". Details: " + ex.getMessage, ex)
      }
    }
  ...
  private def loadPropertyMapping(tnode: TemplateNode, topLevelTemplate: String, context: {
    ...
    tnode.title.decoded match {
      ...
      case "GeocoordinatesMapping" => {
        new GeoCoordinatesMapping(loadOntologyProperty(tnode, "ontologyProperty", required = false, context.ontology),
  ...
  private def loadConditionMapping(tnode: TemplateNode, topLevelTemplate: String, context: {
    ...
    val mapping = tnode.property("mapping").flatMap(mappingNode =>
      mappingNode.children.find(childNode =>
        childNode.isInstanceOf[TemplateNode]))
      .getOrElse(throw new IllegalArgumentException("Condition does not define a mapping"))
      .asInstanceOf[TemplateNode]

    new ConditionMapping(loadTemplateProperty(tnode, "templateProperty", required = false),
      loadTemplateProperty(tnode, "operator", required = false),
      loadTemplateProperty(tnode, "value", required = false),
