 ...
 */
case class ParserFunctionNode(title : WikiTitle, override val children : List[Node], override val line : Int) extends Node(children, line)
{
    def toWikiText() : String = "{{" + title.decoded + ":" + children.map(_.toWikiText()) + "}}"
}
