...
/**
  ...
  */
abstract class HtmlNifExtractor2(nifContextIri: String, language: String, nifParameters : NifParameters) {
  ...
  def extractNif(graphIri: String, subjectIri: String, html: String)(exceptionHandle: (String, RecordSeverity.Value, Throwable) => Unit): Seq[Quad] = {
    ...
    //println(">>>>>>>>>>>"+subjectIri)
//    println("SECTION SIZE >" + sections.size)
    var  context = ""
    var offset = 0
  ...
  ) extends PageSection(section.prev, section.top, section.next, section.sub, section.id, section.title, section.ref, section.tableCount, section.equationCount, section.content)
  {
    ...
    def getExtractedText: String ={
      if(extractedText == null)
        extractedText = HtmlNifExtractor2.ExtractTextFromParagraphs(paragraphs)
      extractedText
    }
