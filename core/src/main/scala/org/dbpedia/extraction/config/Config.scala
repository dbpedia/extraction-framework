...
class Config(properties: Properties) extends Properties(Config.combineProperties(Config.universalProperties, properties))
{
  ...
  /**
    ...
    */
  lazy val provenanceRecordManager: Option[ProvenanceRecordManager] = Try(this.getProperty("record-statement-level-provenance", "false").toBoolean).toOption match{
    case Some(b) if b => Some(new ProvenanceRecordManager(dumpDir, wikiName))
    case _ => None
  }
