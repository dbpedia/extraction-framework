...
object ParserUtilsConfig {
  ...
  /** By default the locale used for number parsing is the language locale.
    * However, this may not be suited for some languages, for instance French
    * where "." is used as a decimal separator instead of "," (because the
    * decimal separator ',' would interfere with template separators) Add an
    * entry when other separators than the locale default are used.
    */
